#!/usr/bin/env bash
# install.sh — Symlink bsm into ~/.local/bin and ~/.claude/skills/bsm
#
# Idempotent: safe to re-run. Existing non-symlink files are backed up as *.bak
# before being replaced.

set -euo pipefail

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

BIN_SRC="$REPO_DIR/bin/bsm"
SKILL_SRC="$REPO_DIR/SKILL.md"

BIN_DST="$HOME/.local/bin/bsm"
SKILL_DST_DIR="$HOME/.claude/skills/bsm"
SKILL_DST="$SKILL_DST_DIR/SKILL.md"

# ── Helpers ──────────────────────────────────────────────────────────────────

_link() {
    local src="$1"
    local dst="$2"

    if [ -L "$dst" ]; then
        # Already a symlink — replace (ln -sfn handles this atomically).
        ln -sfn "$src" "$dst"
        echo "  updated symlink: $dst"
    elif [ -e "$dst" ]; then
        # Real file in the way — back it up before replacing.
        mv "$dst" "${dst}.bak"
        echo "  backed up existing file: ${dst}.bak"
        ln -s "$src" "$dst"
        echo "  created symlink: $dst"
    else
        ln -s "$src" "$dst"
        echo "  created symlink: $dst"
    fi
}

# ── Pre-flight ───────────────────────────────────────────────────────────────

echo "Installing bsm from: $REPO_DIR"
echo ""

if [ ! -x "$BIN_SRC" ]; then
    echo "Error: $BIN_SRC is not executable. Run: chmod +x $BIN_SRC" >&2
    exit 1
fi

if [ ! -f "$SKILL_SRC" ]; then
    echo "Error: $SKILL_SRC not found." >&2
    exit 1
fi

# ── Dependency check ─────────────────────────────────────────────────────────

missing_deps=()
command -v byobu >/dev/null 2>&1 || missing_deps+=("byobu")
command -v jq    >/dev/null 2>&1 || missing_deps+=("jq")

if [ "${#missing_deps[@]}" -gt 0 ]; then
    echo "Warning: missing dependencies: ${missing_deps[*]}"
    echo "         bsm will not work correctly until these are installed."
    echo ""
fi

# ── Install ──────────────────────────────────────────────────────────────────

mkdir -p "$HOME/.local/bin"
mkdir -p "$SKILL_DST_DIR"

echo "Linking bsm script:"
_link "$BIN_SRC" "$BIN_DST"

echo ""
echo "Linking Claude skill definition:"
_link "$SKILL_SRC" "$SKILL_DST"

# ── Post-install instructions ────────────────────────────────────────────────

echo ""
echo "✓ bsm installed."
echo ""
echo "To enable the bsm-* aliases, add this line to your ~/.bashrc (or ~/.zshrc):"
echo ""
echo "    source $REPO_DIR/aliases.sh"
echo ""
echo "Then reload your shell: source ~/.bashrc"
echo ""
echo "Verify with: bsm help"
